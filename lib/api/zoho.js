...
export async function createDeskTicket(hostedPageData, hostedpageId = "") {
  if (!isZohoEnabled()) {
    return { ok: false, disabled: true, reason: "Zoho is not configured yet." };
  }
  ...
  try {
    ...
    console.log(
      JSON.stringify(hostedPageData.subscription.contactpersons[0].phone),
    );
...
<b>Service Product:</b> ${sub.product_name || sub.name || "Culture Wireless Internet"}<br>
