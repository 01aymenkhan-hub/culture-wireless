...
export async function getPlans() {
  ...
  try {
    ...
    // Only call the Add-on API if at least one Fiber plan exists
    if (plans.length > 0) {
      let addonPrice = 0;
      let addonCode = null;
      try {
        ...
        if (addonRes.ok) {
          addonPrice = addonRes.price;
          addonCode = addonRes.addon?.addon_code || "CWG-GSpire";
        } else {
          console.error("Failed to fetch router add-on, falling back to original plan prices:", addonRes.error);
        }
      } catch (addonErr) {
        console.error("Error fetching router add-on:", addonErr.message);
      }
