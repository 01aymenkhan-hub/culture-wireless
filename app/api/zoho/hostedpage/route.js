...
export async function POST(req) {
  ...
  try {
    ...
    const hostedPageId = result.hostedpage?.hostedpage_id || result.hostedpage?.id;
    if (!hostedPageId) {
      console.error("[Hosted Checkout] Zoho response did not include a hosted page ID.");
      return NextResponse.json(
        { ok: false, error: "Unable to initialize checkout. Please try again." },
        { status: 502 },
      );
    }
